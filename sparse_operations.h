...
{	
	...
	IndexType * temp = new_array<IndexType>(num_cols);
...
csr_matrix<IndexType, ValueType>
 csr_transpose(const csr_matrix<IndexType,ValueType>& csr){	
	...
	csr_t.Ap = new_array<IndexType>(csr.num_cols + 1);
	csr_t.Aj = new_array<IndexType>(csr.num_nonzeros);
	csr_t.Ax = new_array<ValueType>(csr.num_nonzeros);
                                   ...
                                   float relative_speed = 3.0, IndexType breakeven_threshold = 4096)
{
    ...
    IndexType * histogram = new_array<IndexType>(max_cols_per_row + 1);
